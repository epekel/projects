...
//	struct s_node *new_node = malloc(sizeof(struct s_node));
	//struct s_node *temp = other_head->next;
	...
	/*if (i = 0) {
	free(other_head); }
		insert_front(head, other_node->value);
		other_node = other_node->next;
	}
	free(other_head);
	struct s_node *new = head->next;
	struct s_node *node_org;
	node_org = new;
	for (i = 0; i <= position; i++) {
		node_org = node_org->next;
	}
	node = node_org;
	for (loop = other_size; loop >= 0; loop--) {
		for (j = 0; j < loop; j++) {
			node = node->next;
			if (j == loop - 1) {
				nodeVal = node->value;
