...
void *phil(void *arg)
{
    ...
    {
		nap();
		confirmR = getfork((long)rightF);
		if (confirmR) {
			nap();
			if (VERBOSE) {
				printf("Philosopher %ld picked up the fork to his right.\n", id);
			}
		}
		confirmL = getfork((long)leftF);
		if (confirmL) {
			nap();
			if (VERBOSE) {
				printf("Philosopher %ld picked up the fork to his left.\n", id);
			}
		}
