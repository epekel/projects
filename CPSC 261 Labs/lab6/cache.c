...
{
    return (cache_line->is_valid && tag == cache_line->tag);
}
...
static cache_line_t *cache_set_find_matching_line(cache_t *cache, cache_set_t *cache_set, intptr_t tag)
{
	...
	unsigned int size = cache->associativity;
	int i;
	for (i = 0; i < size; i++) {
		cache_line = cache_set->cache_lines[i];
		if (cache_line_is_valid_and_both_tags_match(cache_line, tag)) {
			cache_line = cache_line_make_mru(cache_set, i);
			return cache_line;
		}
	}
	return NULL;
...
{
	/* Assumption: Replacement policy is LRU */
...
{
	/* make the address an intptr_t*/
	intptr_t tag = (intptr_t)address >> cache->tag_shift;
	intptr_t set_index = (intptr_t)address >> cache->set_index_shift;
	set_index = set_index & cache->set_index_mask;
	intptr_t block_offset = (intptr_t)address & cache->block_offset_mask;
	long *data;
	unsigned int size = cache->num_sets;
	cache_set_t *sets = cache->sets;
	cache_set_t *cache_set = &sets[set_index];
	cache_line_t *cache_line, *line;
	int i;
	//for (i = 0; i < size; i++) {
	cache_line = cache_set_find_matching_line(cache, cache_set, tag);
	if (cache_line != NULL) {
		data = cache_line_retrieve_data(cache_line, block_offset);
		cache->access_count++;
	}
	else {
		/* data not in cache */
		cache->miss_count++;
		cache->access_count++;
		line = cache_set_add(cache, cache_set, (intptr_t)address, tag);
		data = cache_line_retrieve_data(line, block_offset);
	}
//	}
	return data;
}
